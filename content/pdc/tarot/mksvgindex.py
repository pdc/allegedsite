...
"""
def emitSuitCards(output, suit, startX, startY):
    """Emit SVG for all the cards of the specified suit."""
    for i in range(1, 15):
        dict = { 'BASE' : '', # opts['base'],
                 'SUIT' : suitNames[suit] }
        dict['TEXT'] = rankNames[i - 1] + ' of ' + suitNames[suit]
        x = startX + (i - 1) % cardsPerRow * cardAdvX;
        y = startY + (i - 1) / cardsPerRow* cardAdvY;
...
"""
def emitMenu(output, isSuit, j, startX, startY):
    ...
    for i in range(0, 22, 12):
        dict = { 'BASE'  : '', #: opts['base'],
                 'X' : str(startX),
                 'Y' : str(startY + 20 * (i / 12 + 4)),
                 'FILE' : 'trumps%d' % i,
                 'TEXT' : u'trumps %s\u2013%s' % (rom(i), rom(min(21, i + 12 - 1))) }
        if not isSuit and i == j:
            t = otherTempl
        else:
            t = linkTempl
        output.write(atRe.sub(lambda m, d=dict: dictSub(d, m), t))
