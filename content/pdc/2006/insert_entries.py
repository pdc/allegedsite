...
def update_entry(con, file_name):
	...
	c.execute('insert or replace into Docs (source, title, published) values (?, ?, ?)', (file_name, entry.title, entry.published))
	c.execute('select id from Docs where source = ?', (file_name,))
	doc_id, = c.fetchone()
	c.executemany('insert into Doc_Topics (doc, topic) select ?, id from Topics where name = ?', 
