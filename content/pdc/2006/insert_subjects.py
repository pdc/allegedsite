...
def subjects_iter(file_name):
    ...
    indent_strs = (
        []
    )  # The whitespace from the previous lines, broken at the points the amount of whitespace increased.
...
for ancestors, name, label in subjects_iter(subjects_file):
    ...
    if ancestors:
        c.execute(
            "update Topics set label = ?, parent = (select id from Topics where name = ?) where name = ?",
