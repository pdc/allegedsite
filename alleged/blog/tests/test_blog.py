...
class TestEntry(TestCase):
    ...
    def test_should_munge_relative_href_attribute(self):
        with open(
            os.path.join(self.tmp_dir, "2010/2010-04-18-links.e"), "wt"
        ) as output:
            output.write("Title: Links!\n\nHello [world](17.html)\n")
        entries = get_entries(self.tmp_dir, "/banko/", "/plugh/")
        entry = entries[-1]
        self.assertEqual(
            '<p>Hello <a href="/banko/2010/17.html">world</a></p>', entry.body
        )
    ...
    def test_when_type_attr_included_should_still_munge_script_src(self):
        ...
        entry = entries[-1]
        self.assertEqual(
            '<script type="text/javascript" src="/plugh/2010/SCRIPT.js"></script>',
            entry.body,
        )
    ...
    def test_markdown(self):
        with open(
            os.path.join(self.tmp_dir, "2015/20150802-fictional.e"), "wt"
        ) as out_stream:
            out_stream.write(
                textwrap.dedent(
                    """\
    ...
    def test_markdown_with_links(self):
        with open(
            os.path.join(self.tmp_dir, "2018/2018-09-01-something.e"), "wt"
        ) as output:
            output.write(
                textwrap.dedent(
                    """\
    ...
    def test_href_not_munging_external_link(self):
        ...
        entry = entries[-1]
        self.assertEqual(
            '<p>Hello <a href="http://google.com/">world</a></p>', entry.body
        )
    ...
    def test_src_munging_img_from_markdown(self):
        with open(
            ...
            output.write("Title: Links!\n\nHello ![world](x.jpg)\n")
        entries = get_entries(
            self.tmp_dir, "/banko/", "http://localhost/~pdc/alleged.org.uk/pdc/"
        )
        entry = entries[-1]
        self.assertEqual(
    ...
    def test_find_by_tag(self):
        ...
        self.assertEqual(
            ["a", "e"], [info.tag for info in ae_entries.selected_tag_infos]
        )
    ...
    def test_gets_entry_image_from_header(self):
        with open(
            os.path.join(self.tmp_dir, "2010/2010-04-17-testa.e"), "wt"
        ) as output:
            output.write(
                "Title: FOO\nTopics: alpha beta\nImage: hazmat-64.png\n\nBAR\nBAZ\n"
            )
    ...
    def test_summary_new_style(self):
        ...
        es = get_entries(self.tmp_dir, "/x/", "/i/")
        self.assertEqual(
            '<p>BAR\nBAZ\n<a class="more" href="/x/2003/06/14.html">Read more</a></p>',
            es[0].summary,
        )
                        ...
                        "entries": [
                            {
                                ...
                                "square": {
                                    "href": "http://farm9.staticflickr.com/8483/8229589533_681832d5ef_s.jpg"
                                },
                                "thumbnail": {
                                    "href": "http://farm9.staticflickr.com/8483/8229589533_681832d5ef_t.jpg"
                                },
                                "enclosure": {
    ...
    def check_subset(self, expected, actual, key_prefix):
        for key, expected_val in expected.items():
            ...
            if key not in actual:
                yield "Expected dict to contain key {key_path}".format(
                    key_path=key_path
                )
                break
            ...
            if hasattr(expected_val, "items"):
                ...
                for i, (xv, av) in enumerate(zip(expected_val, actual_val)):
                    for msg in self.check_subset(
                        xv, av, "{key_path}[{i}]".format(key_path=key_path, i=i)
                    ):
                        yield msg
            else:
                if expected_val != actual_val:
                    yield "At key {key_path}, expected {expected!r} but got {actual!r}".format(
                        key_path=key_path, expected=expected_val, actual=actual_val
                    )
...
class TestGithubJsonFromAtom(TestCase):
    ...
    def test_id(self):
        self.assertEqual(
            "tag:github.com,2008:PushEvent/2173402038", self.result["entries"][0]["id"]
        )
    ...
    def test_title(self):
        self.assertEqual(
            "pdc pushed to master at pdc/allegedsite",
            self.result["entries"][0]["title"],
        )
    ...
    def test_content(self):
        ...
        self.assert_text_containing_equivalent_xml(
            expected, self.result["entries"][0]["html"]
        )
...
class TestSummaryFromContent(TestCase):
    ...
    def test_summary_from_content_no_tags(self):
        ...
        self.assertEqual(
            "LiveJournal does not do paragraph tags!", summary_from_content(html)
        )
