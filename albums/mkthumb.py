...
def thumbnail_pipeline(file_name, dimens, meet_or_slice):
    ...
    pipeline = [
        [
            "pamscale",
            ("-xyfit" if meet_or_slice == "meet" else "-xyfill"),
            str(wt),
            str(ht),
        ]
    ]
    ...
    try:
        try:
            opts, args = getopt.getopt(
                argv[1:],
                "mslnd:",
                ["meet", "slice", "overlap", "dry-run", "directory="],
            )
        ...
        for file_name in args[2:]:
            ...
            pipeline = [["djpeg", file_name]] + thumbnail_pipeline(
                file_name, (width, height), mood
            )

            if dir_name:
                out_file = thumbnail_file_name(
                    file_name, dir_name, (width, height), mood
                )
            ...
            if is_dry_run:
                ...
                for cmd in pipeline:
                    if last_process:
                        last_process = Popen(
                            cmd, stdin=last_process.stdout, stdout=PIPE
                        )
                    else:
